...
export async function actualizarCondicionAsist() {
  ...
  try {
    ...
    for (const docSnapshot of querySnapshot.docs) {
      ...
      let coloquiosDesaprobados = notas.filter(n => (n.actividad ?? "").includes("Coloquio Lab.") && n.valor < 6).length;  //(n.valor === "A" || n.valor < 6)
      let informesDesaprobados = notas.filter(n => (n.actividad ?? "").includes("Informe de Lab.") && n.valor < 6).length; //(n.valor === "A" || n.valor < 6)
